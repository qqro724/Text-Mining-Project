# -*- coding: utf-8 -*-
"""망고플레이트 리뷰 감성분석(연남).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1NsLmCNuP-Fy0yy6IeTit9Xz8inlMNEtn
...
df = pd.read_csv('/content/yeonnam cafe review.csv', header=0)
...
for content in zip(y_test[:10], clf.predict(X_test_tfidf[:20]), X_test[:20]):
    print(content)

clf = model_lgbm

# Assuming X_test_tfidf contains your test data and y_test contains your true labels
output_file = 'output.txt'

with open(output_file, 'w', encoding='utf-8') as f:
    for true_label, predicted_label, text_sample in zip(y_test, clf.predict(X_test_tfidf), X_test):
        line = f"True Label: {true_label}, Predicted Label: {predicted_label}, Text: {text_sample}\n"
        f.write(line)
...
plt.bar(range(len(log_clf.coef_[0])), log_clf.coef_[0])

print(sorted(((value, index) for index, value in enumerate(log_clf.coef_[0])), reverse=True)[:5])
print(sorted(((value, index) for index, value in enumerate(log_clf.coef_[0])), reverse=True)[-5:])

"""[중요 피처의 형태소]"""

# 회귀 모델의 계수를 높은 순으로 정렬합니다.
coef_pos_index = sorted(((value, index) for index, value in enumerate(log_clf.coef_[0])), reverse=True)
...
# Print the top 10 positive keywords and their coefficients
top_10_positive_keywords = coef_pos_index[:20]
